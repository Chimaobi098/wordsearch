...
newGame.addEventListener("click",() => {startNewGame()})
 ...
 const filteredDictionary =  dictionary.filter((item) => { return item.length >=4 && item.length <= 5  })
   ...
   let word = [];
      ...
      for(let i = 0; i < wordsToHide; i++){
        ...
        do {
          potentialWord = filteredDictionary[Math.floor(Math.random()*filteredDictionary.length)]
        }
        
        while(word.includes(potentialWord))
        
        word = [...word, potentialWord]
      }
      return word
...
const handleMouseOver = (e) => {
  ...
  e.target.style.color = "white";
    ...
    previousSelectionX = JSON.parse(userSelected[userSelected.length - 1].dataset.position)[0]
    previousSelectionY = JSON.parse(userSelected[userSelected.length - 1].dataset.position)[1]
...
const handleTouch = (e) => {
  ...
  if (userSelected.includes(element) || !element.classList.contains("grid-items")) {
    return;
  } 
console.dir(element)
  if(userSelected.length < 2){
    ...
    let previousSelectionX = JSON.parse(userSelected[userSelected.length - 1].dataset.position)[0]
    let previousSelectionY = JSON.parse(userSelected[userSelected.length - 1].dataset.position)[1]
